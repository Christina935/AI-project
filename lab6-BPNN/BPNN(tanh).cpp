...
void backPropagationEpoc(double rightout,double output,vector<double>hidden_out,vector<double> sample) {
	...
	renew_hw.clear(); 
	double hw = ( rightout- output);
	for (int i = 0;i < hidenode;i++) {
		double hw1 = hw*hidden_out[i];
		renew_hw.push_back(hw1);
	}
//	cout << "输入层-》隐藏层的权值更新："<< endl;
	//输入层->隐藏层的权值更新步长 
	for (int i = 0;i < innode+1;i++) {
		for (int j = 0;j < hidenode;j++) {
			double iw = 1.0*hw*hidden_w[j]  * (1 - hidden_out[j]* hidden_out[j])*sample[i];
//			cout << "input=" << input_w[i][j] << endl;
			input_w[i][j] = input_w[i][j] + 1.0*alpha*iw / (innode+1);
		}
	}
//	cout << "隐藏层->输出层更新后的权值："<< endl; 
	for (int i = 0;i < hidenode;i++) {
		hidden_w[i] = hidden_w[i] + 1.0*alpha*renew_hw[i]/hidenode;
//		cout << hidden_w[i] << endl;
	}
}
...
//	ofstream outFile;
//	outFile.open("F:\\学习资料\\大三上\\人工智能\\实验\\lab6\\result.csv",ios::out);
//	int row_of_res=result.size();
//	for(int r=0;r<row_of_res;r++){
//		outFile << result[r] << "\n";	
//	}
...
//	ofstream out1;
//	out1.open("F:\\学习资料\\大三上\\人工智能\\实验\\lab6\\mse2.csv",ios::out);
...
//	out1.close();
//	ofstream out2;
//	out2.open("F:\\学习资料\\大三上\\人工智能\\实验\\lab6\\mse3.csv",ios::out);
...
//	//20个数据集的预测值和真实值的对比
//	ofstream out1;
//	out1.open("F:\\学习资料\\大三上\\人工智能\\实验\\lab6\\res3.csv",ios::out);
...
//	out1.close();
//	ofstream out2;
//	out2.open("F:\\学习资料\\大三上\\人工智能\\实验\\lab6\\res4.csv",ios::out);
